  ...
  return (
    <div className="newsroom-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="news-submit-button">Submit</button>
      </form>

      <h2 className="existing-items">Existing News Items</h2>
      <div>
        {newsItems.length === 0 ? (
          ...
          <ul className="cards-container">
            {newsItems.map((news) => (
              <li key={news._id} className={`news-card ${newsItems.length === 0 ? "first-card" : ""}`}>
                <div className="news-card-title">{news.title}</div>
                <p className="news-card-content">{news.content}</p>
                <p className="news-card-content">Author: {news.author}</p>
                <button onClick={() => handleDelete(news._id)} className="delete-news">Delete</button>
              </li>
            ))}
          </ul>
        )}
